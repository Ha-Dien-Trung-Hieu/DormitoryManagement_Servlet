...
<head>
    ...
    <style>
       body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #fa66ab, #fbc2eb, #a18cd1); margin: 0; padding: 0; min-height: 100vh; }
		...
		.chat-input-group button:first-child { background: linear-gradient(90deg, #2ecc71, #27ae60); }
		.chat-input-group button:first-child:hover { transform: scale(1.05); background: linear-gradient(90deg, #27ae60, #2ecc71); }
				        ...
				        <div class="form-group chat-form-group">
				            ...
				            <select id="chatWith" onchange="changeChatChannel()">
				                ...
				                <c:forEach var="room" items="${allRooms}">
				                    <option value="room_${fn:escapeXml(room.roomID)}">
				                        Phòng ${fn:escapeXml(room.roomID)}
				                    </option>
				                </c:forEach>
				            </select>
				        </div>
    ...
    <script type="text/javascript">
        ...
        function filterStudents() {
            ...
            for (let i = 0; i < options.length; i++) {
                ...
                const room = option.getAttribute('data-room') ? option.getAttribute('data-room').toLowerCase() : '';
                const value = option.value.toLowerCase();

                if (option.value === '') {
                    continue; // Bỏ qua option mặc định
                }

                if (name.includes(searchInput) || room.includes(searchInput) || value.includes(searchInput)) {
