...
<body>
    <div class="container form-wrapper">
        <h2 class="form-title">Chỉnh Sửa Phòng</h2>
        <c:if test="${not empty errorMessage}">
            <div class="alert alert-danger">${errorMessage}</div>
        </c:if>
        <form action="${pageContext.request.contextPath}/admin/EditRoomServlet" method="post" class="form">
            ...
            <div class="form-group">
                ...
                <select id="roomType" name="roomType" class="form-control">
                    <option value="Đơn" ${room.roomType == 'Đơn' ? 'selected' : ''}>Đơn</option>
                    <option value="Đôi" ${room.roomType == 'Đôi' ? 'selected' : ''}>Đôi</option>
                    <option value="Tập thể" ${room.roomType == 'Tập thể' ? 'selected' : ''}>Tập thể</option>
                </select>
            </div>
            <div class="form-group">
                <label for="capacity">Sức chứa:</label>
                <input type="number" id="capacity" name="capacity" min="1" value="${room.capacity}" class="form-control" required>
            </div>
            <div class="form-group">
                <label for="price">Giá phòng (VNĐ):</label>
                <input type="number" id="price" name="price" min="0" step="100000" value="${room.price}" class="form-control" required>
            </div>
